...
def process_Equi7tile(iOrbitPass, iTileName, iRowStep, iColStep, iChunkSize, oDir):
    ...
    for orbitName, sig0_vv_dc, sig0_vh_dc in zip(my_orbitNames, sig0_vv_dc_list, sig0_vh_dc_list):
        ...
        #------------------------------------------------------------
        # check if all years have data and report years for wich data does not exsist:
        existingYears = annual_vv.count().year.values
        missingYears = np.setdiff1d(np.array([2017, 2018, 2019, 2020]), existingYears)
        if len(missingYears) > 0:
            print('Data are missing for the following years: {}'.format(missingYears))
        #------------------------------------------------------------
        # get groupby MAD per each exsisting year:
        ...
        # calculate statistics only for years in wich data exsists:
        for myYear in existingYears:
            # for VV:
            dist_out_vv_ds['count_' + str(myYear)] = annual_vv.count('time').sel(year=myYear, drop=True)
            dist_out_vv_ds['max_' + str(myYear)] = annual_vv.max('time').sel(year=myYear, drop=True)
            dist_out_vv_ds['min_' + str(myYear)] = annual_vv.min('time').sel(year=myYear, drop=True)
            dist_out_vv_ds['mean_' + str(myYear)] = annual_vv.mean('time').sel(year=myYear, drop=True)
            dist_out_vv_ds['median_' + str(myYear)] = annual_vv.median('time').sel(year=myYear, drop=True)
            dist_out_vv_ds['std_' + str(myYear)] = annual_vv.std('time').sel(year=myYear, drop=True)
            dist_out_vv_ds['q10_' + str(myYear)] = annual_vv.quantile(0.1, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vv_ds['q90_' + str(myYear)] = annual_vv.quantile(0.9, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vv_ds['q25_' + str(myYear)] = annual_vv.quantile(0.25, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vv_ds['q75_' + str(myYear)] = annual_vv.quantile(0.75, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vv_ds['MAD_' + str(myYear)] = MAD_vv.sel(year=myYear, drop=True)
            # for VH:
            dist_out_vh_ds['count_' + str(myYear)] = annual_vh.count('time').sel(year=myYear, drop=True)
            dist_out_vh_ds['max_' + str(myYear)] = annual_vh.max('time').sel(year=myYear, drop=True)
            dist_out_vh_ds['min_' + str(myYear)] = annual_vh.min('time').sel(year=myYear, drop=True)
            dist_out_vh_ds['mean_' + str(myYear)] = annual_vh.mean('time').sel(year=myYear, drop=True)
            dist_out_vh_ds['median_' + str(myYear)] = annual_vh.median('time').sel(year=myYear, drop=True)
            dist_out_vh_ds['std_' + str(myYear)] = annual_vh.std('time').sel(year=myYear, drop=True)
            dist_out_vh_ds['q10_' + str(myYear)] = annual_vh.quantile(0.1, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vh_ds['q90_' + str(myYear)] = annual_vh.quantile(0.9, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vh_ds['q25_' + str(myYear)] = annual_vh.quantile(0.25, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vh_ds['q75_' + str(myYear)] = annual_vh.quantile(0.75, 'time').sel(year=myYear, drop=True).drop('quantile', dim=None)
            dist_out_vh_ds['MAD_' + str(myYear)] = MAD_vh.sel(year=myYear, drop=True)
        #------------------------------------------------------------------------
